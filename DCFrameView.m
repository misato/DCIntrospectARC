...
- (id)initWithFrame:(CGRect)frame delegate:(id)aDelegate
{
	...
	if (self)
	{
		...
		_touchPointLabel.text = @"X 320 Y 480";
		_touchPointLabel.font = [UIFont boldSystemFontOfSize:12.0f];
		_touchPointLabel.textAlignment = UITextAlignmentCenter;
		...
		[self addSubview:_touchPointLabel];
...
- (void)drawRect:(CGRect)rect
{
	...
	if (_rectsToOutline.count > 0)
	{
		for (NSValue *value in _rectsToOutline)
		{
	...
	if (CGRectIsEmpty(_mainRect))
		return;

	CGRect mainRectOffset = CGRectOffset(_mainRect, -_superRect.origin.x, -_superRect.origin.y);
	BOOL showAntialiasingWarning = NO;
	if (! CGRectIsEmpty(_superRect))
	{
		if ((mainRectOffset.origin.x != floorf(mainRectOffset.origin.x) && _mainRect.origin.x != 0) || (mainRectOffset.origin.y != floor(mainRectOffset.origin.y) && _mainRect.origin.y != 0))
	...
	CGRect adjustedMainRect = CGRectMake(_mainRect.origin.x + 0.5f,
										 _mainRect.origin.y + 0.5f,
										 _mainRect.size.width - 1.0f,
										 _mainRect.size.height - 1.0f);
	...
	// edge->left side
	CGContextMoveToPoint(context, CGRectGetMinX(_superRect), floorf(CGRectGetMidY(adjustedMainRect)) + 0.5f);
	...
	[leftDistanceString drawInRect:CGRectMake(CGRectGetMinX(_superRect) + 1.0f,
	...
	// right side->edge
	if (CGRectGetMaxX(_mainRect) < CGRectGetMaxX(_superRect))
	{
		CGContextMoveToPoint(context, CGRectGetMaxX(adjustedMainRect), floorf(CGRectGetMidY(adjustedMainRect)) + 0.5f);
		CGContextAddLineToPoint(context, CGRectGetMaxX(_superRect), floorf(CGRectGetMidY(adjustedMainRect)) + 0.5f);
		CGContextStrokePath(context);
	}
	NSString *rightDistanceString = (showAntialiasingWarning) ? [NSString stringWithFormat:@"%.1f", CGRectGetMaxX(_superRect) - CGRectGetMaxX(adjustedMainRect) - 0.5] : [NSString stringWithFormat:@"%.0f", CGRectGetMaxX(_superRect) - CGRectGetMaxX(adjustedMainRect) - 0.5];
	CGSize rightDistanceStringSize = [rightDistanceString sizeWithFont:font];
	[rightDistanceString drawInRect:CGRectMake(CGRectGetMaxX(_superRect) - rightDistanceStringSize.width - 1.0f,
	...
	// edge->top side
	CGContextMoveToPoint(context, floorf(CGRectGetMidX(adjustedMainRect)) + 0.5f, _superRect.origin.y);
	...
	[topDistanceString drawInRect:CGRectMake(floorf(CGRectGetMidX(adjustedMainRect)) + 3.0f,
											   floorf(CGRectGetMinY(_superRect)),
											   topDistanceStringSize.width,
											   topDistanceStringSize.height)
						   withFont:font];

	// bottom side->edge
	if (CGRectGetMaxY(_mainRect) < CGRectGetMaxY(_superRect))
	{
		CGContextMoveToPoint(context, floorf(CGRectGetMidX(adjustedMainRect)) + 0.5f, CGRectGetMaxY(adjustedMainRect));
		CGContextAddLineToPoint(context, floorf(CGRectGetMidX(adjustedMainRect)) + 0.5f, CGRectGetMaxY(_superRect));
		CGContextStrokePath(context);
	}
	NSString *bottomDistanceString = (showAntialiasingWarning) ? [NSString stringWithFormat:@"%.1f",  CGRectGetMaxY(_superRect) - CGRectGetMaxY(mainRectOffset)] : [NSString stringWithFormat:@"%.0f", _superRect.size.height - mainRectOffset.origin.y - mainRectOffset.size.height];
	...
	[bottomDistanceString drawInRect:CGRectMake(floorf(CGRectGetMidX(adjustedMainRect)) + 3.0f,
												floorf(CGRectGetMaxY(_superRect)) - bottomDistanceStringSize.height - 1.0f,
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	_touchPointLabel.text = touchPontLabelString;

	CGSize stringSize = [touchPontLabelString sizeWithFont:_touchPointLabel.font];
	...
	_touchPointLabel.frame = frame;
	_touchPointView.center = CGPointMake(touchPoint.x + 0.5f, touchPoint.y + 0.5f);
	_touchPointView.alpha = _touchPointLabel.alpha = 1.0f;

	[_delegate touchAtPoint:touchPoint];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	[UIView animateWithDuration:0.08 animations:^{
		_touchPointView.alpha = _touchPointLabel.alpha = 0.0f;
	}];
}
