...
- (void)start
{
	...
	if (!_inputTextView)
	{
		...
		_inputTextView.inputView = [[UIView alloc] init];
		_inputTextView.scrollsToTop = NO;
		[mainWindow addSubview:_inputTextView];
	}
	
	if (_keyboardBindingsOn)
	{
		if (![_inputTextView becomeFirstResponder])
		{
...
- (void)resetInputTextView
{
	_inputTextView.text = @"\n2 4567 9\n";
	_handleArrowKeys = NO;
	_inputTextView.selectedRange = NSMakeRange(5, 0);
	_handleArrowKeys = YES;
}
...
{
	UIWindow *mainWindow = [self mainWindow];
	[mainWindow removeGestureRecognizer:_invokeGestureRecognizer];
	
	_invokeGestureRecognizer = nil;
	_invokeGestureRecognizer = [newGestureRecognizer copy];
...
- (void)touchAtPoint:(CGPoint)point
{
	...
	CGPoint convertedTouchPoint = [[self mainWindow] convertPoint:point fromView:_frameView];
	...
	[_currentViewHistory removeAllObjects];
...
{
	_currentView = view;
	_originalFrame = _currentView.frame;
	_originalAlpha = _currentView.alpha;
	
	if (_frameView.rectsToOutline.count > 0)
	{
		[_frameView.rectsToOutline removeAllObjects];
	...
	if (![_currentViewHistory containsObject:_currentView])
		[_currentViewHistory addObject:_currentView];
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
	...
	_currentView.frame = CGRectMake(floorf(frame.origin.x),
  ...
  if ([string isEqualToString:kDCIntrospectKeysToggleViewOutlines])
	...
	else if ([string isEqualToString:kDCIntrospectKeysToggleShowCoordinates])
	{
		[UIView animateWithDuration:0.15
						 ...
						 animations:^{
							 _frameView.touchPointLabel.alpha = !_frameView.touchPointLabel.alpha;
						 } completion:^(BOOL finished) {
							 NSString *coordinatesString = [NSString stringWithFormat:@"Coordinates are %@", (_frameView.touchPointLabel.alpha) ? @"on" : @"off"];
							 if (_showStatusBarOverlay)
	...
	if (_on && _currentView)
	{
		...
		{
			[self logPropertiesForObject:_currentView];
			return NO;
		}
		else if ([string isEqualToString:kDCIntrospectKeysLogAccessibilityProperties])
		{
			[self logAccessabilityPropertiesForObject:_currentView];
			return NO;
		}
		else if ([string isEqualToString:kDCIntrospectKeysLogViewRecursive])
		{
			[self logRecursiveDescriptionForView:_currentView];
			return NO;
		}
		...
		else if ([string isEqualToString:kDCIntrospectKeysMoveUpInViewHierarchy])
		{
			...
			{
				[self selectView:_currentView.superview];
			}
			else
			{
		...
		{
			if (_currentViewHistory.count == 0)
				return NO;
			
			int indexOfCurrentView = [_currentViewHistory indexOfObject:_currentView];
			...
			[self selectView:[_currentViewHistory objectAtIndex:indexOfCurrentView - 1]];
		}
		else if ([string isEqualToString:kDCIntrospectKeysMoveDownToFirstSubview])
		{
			if (_currentView.subviews.count>0) {
				[self selectView:[_currentView.subviews objectAtIndex:0]];
			}else{
		...
		{
			NSUInteger currentViewsIndex = [_currentView.superview.subviews indexOfObject:_currentView];
			
			if (currentViewsIndex==NSNotFound) {
				NSLog(@"DCIntrospect: BROKEN HIERARCHY.");
			} else if (_currentView.superview.subviews.count>currentViewsIndex + 1) {
				[self selectView:[_currentView.superview.subviews objectAtIndex:currentViewsIndex + 1]];
			}else{
		...
		if ([string isEqualToString:kDCIntrospectKeysNudgeViewLeft])
			...
			frame = CGRectMake(floorf((_currentView.superview.frame.size.width - frame.size.width) / 2.0f),
							   floorf((_currentView.superview.frame.size.height - frame.size.height) / 2.0f),
							   frame.size.width,
							   frame.size.height);
		...
		{
			if (_currentView.alpha < 1.0f)
				_currentView.alpha += 0.05f;
		}
		else if ([string isEqualToString:kDCIntrospectKeysDecreaseViewAlpha])
		{
			if (_currentView.alpha > 0.0f)
				_currentView.alpha -= 0.05f;
		}
...
- (void)logCodeForCurrentViewChanges
{
	...
	NSString *varName = [self nameForObject:_currentView];
	if ([varName isEqualToString:[NSString stringWithFormat:@"%@", _currentView.class]])
		varName = @"<#view#>";
	
	NSMutableString *outputString = [NSMutableString string];
	if (!CGRectEqualToRect(_originalFrame, _currentView.frame))
	{
		[outputString appendFormat:@"%@.frame = CGRectMake(%.1f, %.1f, %.1f, %.1f);\n", varName, _currentView.frame.origin.x, _currentView.frame.origin.y, _currentView.frame.size.width, _currentView.frame.size.height];
	}
	
	if (_originalAlpha != _currentView.alpha)
	{
		[outputString appendFormat:@"%@.alpha = %.2f;\n", varName, _currentView.alpha];
	}
	
	if (outputString.length == 0)
		NSLog(@"DCIntrospect: No changes made to %@.", _currentView.class);
	else
...
- (void)setName:(NSString *)name forObject:(id)object accessedWithSelf:(BOOL)accessedWithSelf
{
	...
	[_objectNames setValue:object forKey:name];
}
...
- (NSString *)nameForObject:(id)object
{
	...
	[_objectNames enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
		if (obj == object)
		{
...
- (void)removeNamesForViewsInView:(UIView *)view
{
	...
	[objectsToRemove enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
		...
		[_objectNames removeObjectForKey:key];
	}];
}
...
- (void)removeNameForObject:(id)object
{
	...
	[_objectNames removeObjectForKey:objectName];
}
...
- (void)updateFrameView
{
	...
	if (!_frameView)
	{
		...
		[mainWindow addSubview:_frameView];
		_frameView.alpha = 0.0f;
		[self updateViews];
	}
	
	[mainWindow bringSubviewToFront:_frameView];
	
	if (_on)
	{
		if (_currentView)
		{
			_frameView.mainRect = [_currentView.superview convertRect:_currentView.frame toView:_frameView];
			if (_currentView.superview == mainWindow)
				_frameView.superRect = CGRectZero;
			else if (_currentView.superview.superview)
				_frameView.superRect = [_currentView.superview.superview convertRect:_currentView.superview.frame toView:_frameView];
			else
				_frameView.superRect = CGRectZero;
		}
		else
		{
			_frameView.mainRect = CGRectZero;
		}
		
		[self fadeView:_frameView toAlpha:1.0f];
	}
	else
	{
		[self fadeView:_frameView toAlpha:0.0f];
	}
}
...
- (void)updateStatusBar
{
	if (_currentView)
	{
		NSString *nameForObject = [self nameForObject:_currentView];
		...
		if (_currentView.tag != 0)
			_statusBarOverlay.leftLabel.text = [NSString stringWithFormat:@"%@ (tag: %i)", nameForObject, _currentView.tag];
		else
			_statusBarOverlay.leftLabel.text = [NSString stringWithFormat:@"%@", nameForObject];
		
		_statusBarOverlay.rightLabel.text = NSStringFromCGRect(_currentView.frame);
	}
	else
	{
		_statusBarOverlay.leftLabel.text = @"DCIntrospect";
		_statusBarOverlay.rightLabel.text = [NSString stringWithFormat:@"'%@' for help", kDCIntrospectKeysToggleHelp];
	}
...
- (void)updateViews
{
	...
	{
		_frameView.transform = CGAffineTransformIdentity;
		_frameView.frame = CGRectMake(0, 0, screenWidth, screenHeight);
	}
	else if (orientation == UIDeviceOrientationLandscapeLeft)
	{
		_frameView.transform = CGAffineTransformMakeRotation(pi * (90) / 180.0f);
		_frameView.frame = CGRectMake(screenWidth - screenHeight, 0, screenHeight, screenHeight);
	}
	...
	{
		_frameView.transform = CGAffineTransformMakeRotation(pi);
		_frameView.frame = CGRectMake(0, 0, screenWidth, screenHeight);
	}
...
- (void)showTemporaryStringInStatusBar:(NSString *)string
{
	...
	_statusBarOverlay.leftLabel.text = string;
	_statusBarOverlay.rightLabel.text = nil;
...
{
	if ([_currentView class] == [UITableView class])
		[(UITableView *)_currentView reloadData];
}

- (void)toggleOutlines
{
	UIWindow *mainWindow = [self mainWindow];
	_viewOutlines = !_viewOutlines;
	...
	NSString *string = [NSString stringWithFormat:@"Showing view outlines is %@", (_viewOutlines) ? @"on" : @"off"];
	if (_showStatusBarOverlay)
...
- (void)addOutlinesToFrameViewFromSubview:(UIView *)view
{
	for (UIView *subview in view.subviews)
	{
		...
		CGRect rect = [subview.superview convertRect:subview.frame toView:_frameView];
		
		NSValue *rectValue = [NSValue valueWithCGRect:rect];
		[_frameView.rectsToOutline addObject:rectValue];
...
{
	_highlightNonOpaqueViews = !_highlightNonOpaqueViews;
	
	UIWindow *mainWindow = [self mainWindow];
	[self setBackgroundColor:(_highlightNonOpaqueViews) ? kDCIntrospectOpaqueColor : [UIColor clearColor]
   ofNonOpaqueViewsInSubview:mainWindow];
	
	NSString *string = [NSString stringWithFormat:@"Highlighting non-opaque views is %@", (_highlightNonOpaqueViews) ? @"on" : @"off"];
	if (_showStatusBarOverlay)
		[self showTemporaryStringInStatusBar:string];
	else
		NSLog(@"DCIntrospect: %@", string);
...
{
	_flashOnRedraw = !_flashOnRedraw;
	NSString *string = [NSString stringWithFormat:@"Flashing on redraw is %@", (_flashOnRedraw) ? @"on" : @"off"];
	if (_showStatusBarOverlay)
		[self showTemporaryStringInStatusBar:string];
	else
		NSLog(@"DCIntrospect: %@", string);
...
- (void)toggleHelp
{
	...
	if (_showingHelp)
	{
		_statusBarOverlay.leftLabel.text = @"Help";
		_statusBarOverlay.rightLabel.text = @"Any key to close";
...
{
	if (view == _frameView || view == _inputTextView)
		return YES;
	return NO;
}
