...
- (id)init
{
    if ((self = [super initWithFrame:CGRectZero]))
	{
		...
		_leftLabel.font = [UIFont boldSystemFontOfSize:12.0f];
		_leftLabel.textColor = [UIColor colorWithWhite:0.97f alpha:1.0f];
		_leftLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
		[self addSubview:_leftLabel];

		self.rightLabel = [[UILabel alloc] initWithFrame:CGRectOffset(self.frame, -2.0f, 0.0f)];
		_rightLabel.backgroundColor = [UIColor clearColor];
		_rightLabel.font = [UIFont boldSystemFontOfSize:12.0f];
		_rightLabel.textAlignment = UITextAlignmentRight;
		_rightLabel.textColor = [UIColor colorWithWhite:0.9f alpha:1.0f];
		_rightLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
